...
struct Profile: Identifiable, Codable, Equatable {
    ...
    init(
        ...
        fileName: String = "",
        notes: String? = nil,
        userAgent: String = "ClashForMacOS/1.0",
        updateInterval: Int = 1440,
        useSystemProxy: Bool = false,
        useClashProxy: Bool = false,
        lastAutoUpdate: Date? = nil
    ) {
        self.id = id
        self.name = name
        self.type = type
...
class ProfileManager {
    ...
    var profilesDirectory: URL {
        let dir = ClashCoreManager.shared.appSupportDirectory
    ...
    private var profilesConfigPath: URL {
        ClashCoreManager.shared.appSupportDirectory.appendingPathComponent(
            "profiles.json"
        )
    }
    ...
    func startAutoUpdateTimer() {
        ...
        autoUpdateTimer = Timer.scheduledTimer(
            withTimeInterval: 60,
            repeats: true
        ) { [weak self] _ in
            Task { @MainActor in
    ...
    func checkAndUpdateProfiles() async {
        ...
        where profile.type == .remote && profile.updateInterval > 0 {
    ...
    func loadProfiles() {
        guard fileManager.fileExists(atPath: profilesConfigPath.path) else {
            return
        }
            ...
            do {
                ...
                await MainActor.run {
                    downloadStatus = .failed(
                        "Invalid YAML: \(error.localizedDescription)"
                    )
                }
                return false
            }
    ...
    @discardableResult
    func updateProfile(_ profile: Profile, isAutoUpdate: Bool = false) async
        -> Bool
    {
        guard profile.type == .remote, let urlString = profile.url else {
            return false
        }
        ...
        do {
            ...
            let filePath = profilesDirectory.appendingPathComponent(
                profile.fileName
            )
            try normalizedContent.write(
                to: filePath,
    ...
    private func extractProfileName(
