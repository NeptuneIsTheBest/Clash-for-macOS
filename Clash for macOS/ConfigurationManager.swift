...
class ConfigurationManager {
    ...
    func syncConfiguration() {
        if let selectedId = ProfileManager.shared.selectedProfileId,
            let profile = ProfileManager.shared.profiles.first(where: {
                $0.id == selectedId
            })
        {
    ...
    private func applyProfileWithSettings(_ profile: Profile) {
        let sourcePath = ProfileManager.shared.profilesDirectory
    ...
    private func mergeWithGeneralSettings(_ profileContent: String) -> String {
        ...
        guard
            var config = try? Yams.load(yaml: profileContent) as? [String: Any]
        else {
        ...
        guard let result = try? Yams.dump(object: config, allowUnicode: true)
        else {
            return profileContent
        }
