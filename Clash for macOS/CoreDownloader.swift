...
class CoreDownloader {
    ...
    func download(coreType: ClashCoreType, to destinationURL: URL) async -> (success: Bool, version: String?) {
        guard !isDownloading else { return (false, nil) }
        
        status = .downloading(progress: 0.1)
        
        do {
            let (version, downloadURL, fileName) = try await fetchLatestReleaseInfo(for: coreType)
    ...
    private func fetchLatestReleaseInfo(for coreType: ClashCoreType) async throws -> (version: String, downloadURL: URL, fileName: String) {
        let urlString = "https://api.github.com/repos/\(coreType.repoPath)/releases/latest"
    ...
    private func getMatchRules(for coreType: ClashCoreType) -> (keywords: [String], suffix: String?) {
        let arch = systemArch
