...
class HelperManager {
    ...
    private let helperPath =
        "/Library/PrivilegedHelperTools/Clash for macOS Helper"
    ...
    func checkHelperStatus() {
        let service = SMAppService.daemon(
            plistName: "com.neptuneisthebest.Clash-for-macOS-Helper.plist"
        )
    ...
    private func loadEmbeddedHelperVersion() {
        ...
        let possibleURLs = [
            bundleURL.appendingPathComponent(
                "Contents/Library/LaunchServices/\(helperName)"
            ),
            bundleURL.appendingPathComponent(
                "Contents/Library/LaunchDaemons/\(helperName)"
            ),
        ]
        ...
        if SecStaticCodeCreateWithPath(url as CFURL, [], &staticCode)
            == errSecSuccess,
            let code = staticCode
        {
            ...
            if SecCodeCopySigningInformation(code, [], &info) == errSecSuccess,
                ...
                let plist = infoDict[kSecCodeInfoPList as String]
                    as? [String: Any],
    ...
    private func checkIfUpdateNeeded() {
        ...
        helperNeedsUpdate =
            compareVersions(helperVersion, embeddedHelperVersion)
            == .orderedAscending
    }
    ...
    private func getConnection() -> NSXPCConnection {
        ...
        let connection = NSXPCConnection(
            machServiceName: kHelperToolMachServiceName,
            options: .privileged
        )
        let interface = NSXPCInterface(with: HelperProtocol.self)
        let expectedClasses =
            NSSet(objects: NSArray.self, NSString.self) as! Set<AnyHashable>
        interface.setClasses(
            expectedClasses,
            for: #selector(
                HelperProtocol.runPrivilegedCommand(
                    command:
                    arguments:
                    withReply:
                )
    ...
    func startClashCore(
        ...
        completion: @escaping (Bool, Int32, String?) -> Void
    ) {
        guard let helper = getHelper() else {
            completion(false, 0, "Helper not available")
            return
        }
        helper.startClashCore(
            executablePath: executablePath,
            configPath: configPath,
            workingDirectory: workingDirectory
        ) { success, pid, error in
            DispatchQueue.main.async {
    ...
    func runCommand(
        ...
        helper.runPrivilegedCommand(command: command, arguments: arguments) {
            success,
            output,
            error in
            DispatchQueue.main.async {
