    ...
    @unchecked Sendable
{
    ...
    func setSystemProxy(
        ...
        for service in networkServices {
            if !httpPort.isEmpty {
                let httpResult = runNetworkSetup([
                    "-setwebproxy", service, host, httpPort,
                ])
                if !httpResult.success { lastError = httpResult.error }

                let httpsResult = runNetworkSetup([
                    "-setsecurewebproxy", service, host, httpPort,
                ])
                if !httpsResult.success { lastError = httpsResult.error }
            }

            if !socksPort.isEmpty {
                let socksResult = runNetworkSetup([
                    "-setsocksfirewallproxy", service, host, socksPort,
                ])
            ...
            if !bypassDomains.isEmpty {
                ...
                let bypassResult = runNetworkSetup(
    ...
    func clearSystemProxy(withReply reply: @escaping (Bool, String?) -> Void) {
        ...
        for service in networkServices {
            ...
            let httpsOff = runNetworkSetup([
                "-setsecurewebproxystate", service, "off",
            ])
            if !httpsOff.success { lastError = httpsOff.error }

            let socksOff = runNetworkSetup([
            ...
            DispatchQueue.global().asyncAfter(deadline: .now() + 1.0) {
                ...
                if isRunning {
                    ...
                    let finalError =
                        ...
                        ? errorString : "Process exited unexpectedly"
